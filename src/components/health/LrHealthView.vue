...
<template>
  <div class="pa-5" v-feature="unsupportedFeatures.health">
    <div class="cortx-text-lg cortx-text-bold pr-2">
      {{ $t("health.healthView") }}
    </div>
    <div>
      <div class="cortx-health-summary-container">
        <div class="cortx-text-lg cortx-float-l cortx-text-bold">
          {{ componentName }}
        </div>
        <div
        ...
        >
          <div class="summary-count">
            <label class="cortx-text-sm">{{
              healthSummary.good ? healthSummary.good : 0
            }}</label>
          </div>
        </div>
        <div
        ...
        >
          <div class="summary-count">
            <label class="cortx-text-sm">{{
              healthSummary.warning ? healthSummary.warning : 0
            }}</label>
          </div>
        </div>
        <div
        ...
        >
          <div class="summary-count">
            <label class="cortx-text-sm">{{
              healthSummary.critical ? healthSummary.critical : 0
            }}</label>
          </div>
        </div>
      </div>
...
export default class CortxHealthView extends Vue {
  ...
  public async mounted() {
    ...
    const enclosureName = this.$route.query.name ? this.$route.query.name : "";
    const res = await Api.getAll(apiRegister.health_components, {
      node_id: enclosureName,
    });
    if (res && res.data) {
      this.healthComponentData =
        res.data[0][Object.keys(res.data[0])[0]].components;
      this.healthSummary =
  ...
  get componentName() {
    ...
    if (this.$route.query.name) {
      compName =
        this.$route.query.name === "storage_encl"
