...
*/
import { ResourceConfig, COLOR_CODES, STATUS_CONTAINER_WIDTH } from "./cortx-health-config";
import * as d3 from "d3";
import * as moment from "moment";

export default abstract class HealthCardBuilder {

    public static build(healthCardG: any, expandCollapse: any, performAction: any): any {
    ...
    public static buildOuterRectangle(healthCardG: any) {
        healthCardG.append("rect").attr("x", 0.500488).attr("y", 0.5)
            .attr("width", 269).attr("height", 119)
            .attr("rx", 3.5).attr("fill", "#FFFFFF")
            .attr("stroke", (data: any) => data.config.color.dark);
    }

    public static buildHeaderPath(healthCardG: any) {
        healthCardG.append("path")
            .attr("d", "M4.00049 0.5H266C267.934 0.5 269.5 2.067 269.5 4V49.5H0.500488V4C0.500488 2.067 2.06749 0.5 4.00049 0.5Z")
            .attr("fill", (data: any) => data.config.color.light)
            .attr("stroke", (data: any) => data.config.color.dark);
    }

    public static buildIconPlaceHolderRectangle(healthCardG: any) {
        healthCardG.append("rect")
            .attr("x", 15.5)
            .attr("y", 60.5)
            .attr("width", 49)
            .attr("height", 49)
    ...
    public static buildIcon(healthCardG: any) {
        healthCardG.append("image")
            .attr("xlink:href", (data: any) => require(`@/assets/health/${data.resource}/${data.status}/icon.svg`))
            .attr("x", 21.5)
            .attr("y", 66.5)
            .attr("width", 37)
            .attr("height", 37);
    ...
    public static buildShowActionsIcon(healthCardG: any, performAction: any) {
        healthCardG.append("image")
            ...
            .attr("width", 10)
            .attr("height", 25)
            .attr("visibility", (data: any) => data.config.actions.length > 0 ? "visible" : "hidden")
            .attr("style", "cursor: pointer;")
            .on("click", (data: any) => {
    ...
    public static buildDetailsMenu(data: any) {
        ...
        const lastUpdatedTime = moment.default(lastUpdatedTimeInMillis).format("DD-MM-YYYY hh:mm A");
        const details: any[] = [
            {
                title: "Resource",
        ...
        const detailsMenuContainerG = outerG.append("g")
        ...
        detailsMenuContainerG.append("rect").attr("x", 0).attr("y", 0)
            .attr("height", detailsMenuContainerHeight)
            .attr("width", 269)
            .attr("fill", "#FFFFFF")
        ...
        details.forEach((detail) => {
            const detailItemContainerG: any = detailsMenuContainerG.append("g")
                                                .attr("transform", `translate(0.5,${detailItemY})`)
                                                .attr("width", 268)
                                                .attr("height", 35);
            detailItemContainerG.append("text")
                .attr("x", 15)
                .attr("y", 20)
                .attr("fill", "#000000")
    ...
    public static buildActionsMenu(data: any, performAction: any) {
        ...
        if (actions.length > 0) {
            ...
            const actionsMenuContainerG = outerG.append("g")
            ...
            actionsMenuContainerG.append("rect").attr("x", 0).attr("y", 0)
                .attr("height", actionsMenuContainerHeight)
                .attr("width", 269)
                .attr("fill", "#FFFFFF")
                .attr("stroke", data.config.color.dark);
            ...
            actions.forEach((action: any) => {
                const actionItemContainerG: any = actionsMenuContainerG.append("g")
                                                    .attr("transform", `translate(0.5,${actionItemY})`)
                                                    .attr("width", 268)
                                                    .attr("height", 40);
                const rectId: string = `rect_${data.treeNodeId}_${action.action}`;
                actionItemContainerG.append("text")
                    .attr("x", 45)
                    .attr("y", 25)
                    .attr("fill", "#000000")
                    .attr("style", "font-size: 16px;")
                    .text(action.title);
                actionItemContainerG.append("image")
                    .attr("xlink:href", require(`@/assets/health/${data.resource}/${data.status}/operations/${action.action}.svg`))
                    .attr("x", 15)
                    .attr("y", 11)
                    .attr("width", 22)
                    .attr("height", 22);

                const actionItemRect: any = actionItemContainerG.append("rect")
                                                .attr("id", rectId)
                                                .attr("height", 40)
                                                .attr("width", 268)
                                                .attr("fill", "#E5E5E5");
    ...
    public static buildConfig(resource: any) {
        const actions: any = ResourceConfig.getActionsOfResource(resource.resource, resource.status);
        resource.config = {
