<template >
  <v-container class="mt-6 ml-6">
    ...
    <div class="body-2">
      ...
      <div
        class="mt-2"
       id="lblNSMsg">Use the following screens to finalize your management network settings for mapping the system. You can set system to be managed in an IPv4 network, an IPv6 network or both. You can skip this section entirely if your network settings are complete.</div>
      <div class="mt-6">Choose which network settings you'd like to establish.</div>
      <v-divider class="mt-2" />
      <div class="mt-8">
        <input type="checkbox" name="ipv4" :disabled="isSkip" v-model="isipV4Status" @change="isipV4Status" id="chkNsisipV4Status" />
      ...
      <div class="mt-6">
        <input type="checkbox" @change="isipV6Status" :disabled="isSkip" v-model="isipV6Status" name="ipv6" id="chkNsisipV6Status" />
        <span class="ml-4 font-weight-medium">IPv6</span>
      </div>
      <div
        class="mt-1"
      id="lblNsIp6Msg" >Selecting IPv6 will allow you to view settings assigned by DHCP or to assign static IPv6 data network settings for environments that do not support DHCP.</div>
      <div class="mt-6">
        <input
          type="checkbox"
          name="skip"
