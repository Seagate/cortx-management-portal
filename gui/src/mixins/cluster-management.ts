...
export default class ClusterManagementMixin extends Vue {
    ...
    public async getHealthData(isTreeFormat: boolean) {
        ...
        if (isTreeFormat) {
            ...
            const res: any = await Api.getAll(apiRegister.health_cluster, healthQueryParams);
    ...
    public async promptDialogClosed(prompt: string) {
        ...
        if (prompt === "yes") {
            this.$store.dispatch("systemConfig/showLoader", this.operationRequestObj.loaderMsg);
            const resp: any = await Api.post(`${apiRegister.manage_resource}/${this.operationRequestObj.resource}`,
    ...
    public async getNodeStopClusterStatus(node_id: string) {
        this.$store.dispatch("systemConfig/showLoader", "Fetching cluster status...");
        const res: any = await Api.getAll(`${apiRegister.cluster_status}/${node_id}`);
        this.$store.dispatch("systemConfig/hideLoader");
        return res.data;
    }

    public buildOperationRequestObj(resource: string, loaderMsg: string, requestObj: any) {
